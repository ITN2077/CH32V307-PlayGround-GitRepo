...
motor_recip_instance_t *P_M1_recip_instance = NULL; // 添加往复运动实例指针
...
void PeripheraAll_Init()
{
    ...
    // 初始化电机往复运动实例
    P_M1_recip_instance = motor_recip_instance_create(motor1_step_instance_start, motor1_step_instance_stop);

    // 默认启动往复运动作为测试
    motor_recip_instance_start(P_M1_recip_instance, 100, 2000, 2000, 500); // 占空比100，正向200ms，反向200ms，冷却50ms
...
void While_Task(void)
{
    // 写入while循环任务
    // 测试往复运动功能
    uint16_t forward_duration_ms = 2000;
    uint16_t backward_duration_ms = 2000;
    uint16_t cooldown_duration_ms = 500;

    // 当test_value_1大于0时，启动往复运动
    if (test_value_1 > 0)
    {
        motor_recip_instance_start(P_M1_recip_instance, 100, forward_duration_ms, backward_duration_ms, cooldown_duration_ms);
        // printf_USART_DEBUG("往复运动启动：正向%dms，反向%dms，冷却%dms\r\n", forward_duration_ms, backward_duration_ms, cooldown_duration_ms);
    }
    // 当test_value_1小于0时，停止往复运动
    else if (test_value_1 == 0)
    {
        motor_recip_instance_stop(P_M1_recip_instance);
        //printf_USART_DEBUG("往复运动停止\r\n");
    }
}
/**
 *  @brief 串口数据包处理任务函数
