...
void Time_Slice_Offset_Register(void)
{
    ...
    XxxTimeSliceOffset_Register(&While_task, While_Task, 1, 0);             // 注册while循环任务。
...
void PeripheraAll_Init()
{
    ...
    printf_USART_DEBUG("hello,WSY&DSX!\r\n");
    pwm_init(TIM9_PWM_MAP0_CH3_A4, 490, 500);
...
{
    // 写入while循环任务
    pwm_set_duty(TIM9_PWM_MAP0_CH3_A4, test_value_1);
}
/**
 *  @brief 串口数据包处理任务函数
