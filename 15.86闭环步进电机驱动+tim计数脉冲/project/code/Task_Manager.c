...
void PeripheraAll_Init()
{
    ...
    pwm_init(TIM9_PWM_MAP0_CH3_A4, 2, 5000);         // 硬件pwm输出脉冲

    // // 配置TIM9更新中断
    TIM_ITConfig(TIM9, TIM_IT_Update, ENABLE); // 使能TIM9更新中断
    NVIC_EnableIRQ(TIM9_UP_IRQn);              // 使能TIM9中断通道
    NVIC_SetPriority(TIM9_UP_IRQn, 1);         // 设置中断优先级
