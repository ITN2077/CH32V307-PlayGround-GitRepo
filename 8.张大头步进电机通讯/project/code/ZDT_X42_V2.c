...
平台初始化：
  ...
  void platform_uart_send(uint8_t *data, uint16_t length) {
      // 具体的串口发送实现
  ...
  ZDT_UartInterface_t uart_if = {
      ...
      .send_data = platform_uart_send
  };

  // 初始化模块
  ZDT_X42_V2_Init(&uart_if);
**********************************************************/

// 静态变量存储串口接口
static const ZDT_UartInterface_t *g_uart_if = (void *)0;
...
static void UART_SEND(uint8_t *data, uint16_t length)
{
  ...
  {
    g_uart_if->send_data(data, length);
  }
}
