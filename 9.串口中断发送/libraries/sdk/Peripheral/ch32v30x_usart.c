 ...
 * Version            : V1.0.0
 * Date               : 2021/06/06
 * Description        : 本文件提供了所有USART固件功能函数。
...
#define CTLR2_LBDL_Mask ((uint16_t)0xFFDF)        /* USART LIN断点检测掩码 */
#define CTLR2_STOP_CLEAR_Mask ((uint16_t)0xCFFF)  /* USART CR2停止位掩码 */
#define CTLR2_CLOCK_CLEAR_Mask ((uint16_t)0xF0FF) /* USART CR2时钟掩码 */

#define CTLR3_SCEN_Set ((uint16_t)0x0020)   /* USART智能卡使能掩码 */
#define CTLR3_SCEN_Reset ((uint16_t)0xFFDF) /* USART智能卡禁用掩码 */

#define CTLR3_NACK_Set ((uint16_t)0x0010)   /* USART智能卡NACK使能掩码 */
#define CTLR3_NACK_Reset ((uint16_t)0xFFEF) /* USART智能卡NACK禁用掩码 */
...
/* USART单比特采样掩码 */
#define CTLR3_ONEBITE_Set ((uint16_t)0x0800)   /* USART单比特模式使能掩码 */
#define CTLR3_ONEBITE_Reset ((uint16_t)0xF7FF) /* USART单比特模式禁用掩码 */

/*********************************************************************
 * @fn      USART_DeInit
 *
 * @brief   将USARTx外设寄存器初始化为默认复位值。
 *
 * @param   USARTx - 其中x可以是1、2或3，用于选择UART外设。
 *
 * @return  无
 */
 ...
 * @fn      USART_StructInit
 *
 * @brief   将每个USART_InitStruct成员填充为其默认值。
 *
 * @param   SPIx - 其中x可以是1、2或3，用于选择SPI外设。
 *
 * @return  无
 */
 ...
 *
 * @param   USARTx - 其中x可以是1、2、3，用于选择USART外设。
 ...
 *
 * @brief   将每个USART_ClockStructInit成员填充为其默认值。
 *
 * @param   USART_ClockInitStruct - 指向USART_ClockInitTypeDef结构的指针，该结构将被初始化。
 *
 * @return  无
 */
 ...
 *
 * @brief   设置USART LIN断点检测长度。
 *
 * @param   USARTx - 其中x可以是1、2、3，用于选择USART外设。
 *          USART_LINBreakDetectLength - 指定LIN断点检测长度。
 *            USART_LINBreakDetectLength_10b - 10位断点检测。
 *            USART_LINBreakDetectLength_11b - 11位断点检测。
 *
 * @return  无
 */
...
{
    /* 从USART数据寄存器(DATAR)中读取数据，并屏蔽掉高7位(只保留低9位)
     * 0x01FF = 0000 0001 1111 1111，用于确保只获取有效数据位
     * 因为USART支持8位或9位数据长度，所以需要保留9位数据 */
 ...
 *
 * @brief   检查指定的USART标志是否设置。
 *
 * @param   USARTx - 其中x可以是1、2、3，用于选择USART外设。
 *          USART_FLAG - 指定要检查的标志。
 *            USART_FLAG_CTS - CTS变化标志。
 *            USART_FLAG_LBD - LIN断点检测标志。
