...
void UART_DEBUG_Init(void)
{
    ...
    uart_init(DEBUG_UART_INDEX, DEBUG_UART_BAUDRATE, DEBUG_UART_TX_PIN, DEBUG_UART_RX_PIN); // 串口初始化 - 此函数为平台相关函数，用于初始化串口硬件
    uart_rx_interrupt(DEBUG_UART_INDEX, ENABLE);                                            // 开启接收中断 - 此函数为平台相关函数，用于使能串口接收中断
    interrupt_set_priority(UART8_IRQn, (0 << 5) || 1);                                     // 设置usart3的中断优先级 - 此函数为平台相关函数，用于设置中断优先级
 ...
 *  @warning 在UART_Data_Unpacker.h中声明，在isr.c对应串口号引用
 */
