...
void ZDT_X42_V2_Read_Sys_Params(uint8_t addr, SysParams_t s)
{
  ...
  if(s >= S_Conf)
  {
    cmd[3] = 0x6B; ZDT_uart_send_data(cmd, 4);
  }
  else
  {
    cmd[2] = 0x6B; ZDT_uart_send_data(cmd, 3);
  }
}
 ...
 *  @param
 *  @return
 *  @note 使用串口七，TX：E12，RX：E13，波特率115200，8-N-1，无流控
 *  @warning
 */
 ...
 *  @warning 无
 */
void ZDT_uart_send_data(uint8_t *data, uint16_t length)
{
