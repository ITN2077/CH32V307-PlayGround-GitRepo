...
void PeripheraAll_Init()
{
    ...
    Motor_Init(&motor_m4);
    Motor_Init(&motor_m5);
    Encoder_Init(&encoder_e3);
    Encoder_Init(&encoder_e4);
    LimitSwitch_Init(&limit_ls1u);
    LimitSwitch_Init(&limit_ls1d);
    LimitSwitch_Init(&limit_ls2d);
    LimitSwitch_Init(&limit_ls3);
    LimitSwitch_Init(&limit_ls4);
    weight_sensor_ttl_uart_init();
    zero_calibration(&sp3_sensor);
    zero_calibration(&sp4_sensor);
    printf_USART_DEBUG("sp3_sensor.zero_value: %d\r\n", sp3_sensor.zero_value);
    printf_USART_DEBUG("sp4_sensor.zero_value: %d\r\n", sp4_sensor.zero_value);
...
{
    WeightSensor_ReadAD(&sp3_sensor);
    printf_USART_DEBUG("sp3_sensor.data.value: %d\r\n", sp3_sensor.data.value);
    printf_USART_DEBUG("sp3_sensor.pressure: %d\r\n", sp3_sensor.data.pressure);

    WeightSensor_ReadAD(&sp4_sensor);
    printf_USART_DEBUG("sp4_sensor.data.value: %d\r\n", sp4_sensor.data.value);
    printf_USART_DEBUG("sp4_sensor.pressure: %d\r\n", sp4_sensor.data.pressure);
    // 写入while循环任务
}
/**
 *  @brief 串口数据包处理任务函数
...
void UART_packet_TASKhandler(void)
{
    ...
    WeightSensor_ParseFromRingBuffer(&weight_sensor_ttl_uart_rx_buffer, &sp3_sensor);
    WeightSensor_ParseFromRingBuffer(&weight_sensor_ttl_uart_rx_buffer, &sp4_sensor);
}
/**
